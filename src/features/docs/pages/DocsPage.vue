...
const structures: InfoItem[] = [
  {
    ...
    name: 'src/app',
    description: '应用组合层：routes、layout、providers、notifications、navigation、global styles。',
  },
  {
    icon: FolderIcon,
    name: 'src/features',
  ...
  <main class="min-h-screen bg-background text-foreground">
    <section class="mx-auto flex w-full max-w-6xl flex-col gap-8 px-6 py-10">
      <header class="flex flex-col gap-4">
        <Badge class="w-fit" variant="secondary">
          Vue Template Guide
        </Badge>
        <div class="flex flex-col gap-3">
          ...
          <p class="max-w-3xl text-base leading-7 text-muted-foreground">
            这是给开发者看的极简索引。模板改为 app/features/shared 分层，UI 使用 shadcn-vue，样式使用 Tailwind CSS。
          </p>
        </div>
      </header>
