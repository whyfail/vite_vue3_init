...
function hasComponentNameReference(testContent, componentName) {
  ...
  const patterns = [
    ...
    new RegExp(`components:\\s*\\{[^}]*\\b${escapedName}\\b[^}]*\\}`),
    new RegExp(`<${escapedName}(\\s|/|>)`),
    new RegExp(`mount\\([^)]*\\b${escapedName}\\b`),
    new RegExp(`mountWithRouter\\([^)]*\\b${escapedName}\\b`),
  ];
...
const components = componentDirs.flatMap((dir) =>
  walk(dir, (filePath) => componentExtensions.has(extname(filePath))),
);
